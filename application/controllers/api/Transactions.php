	...
	public function Process_post(){
		...
		if($authentication==true){
			...
			foreach($getApiList as $apiInfo){
				if($apiInfo->api_name=='NMI' && $transDone == 0){
					...
					if($this->input->post('currency')=='GBP' || $this->input->post('currency')=='EUR'){
						$apiInfo = $this->transactions_model->getUserAPIByCurrency($merchantId, 'cc', $apiData['currency']);
						//getPrint($apiInfo);
					}
					
                    $apiResponse = $this->paytoo->doTransaction($apiData, $apiInfo);
					$nameOfApiForTrans	= $apiInfo->api_name;
					$transApiID			= $apiInfo->api_id;	
					$transactionMode	= $apiInfo->api_mode;
			...
			if($nameOfApiForTrans=='NMI'){
				...
				$message['transaction_request_id']	= $transData['transaction_request_id'];
				$message['transaction_key']			= $transData['transaction_key'];
