	...
	public function achpayment(){
		...
		if ($this->form_validation->run() == false){
			...
			foreach($getApiList as $apiInfo){
				...
				if($apiInfo->api_name=='Khitomer' && $transDone == 0){
				...
				}elseif($apiInfo->api_name=='Khitomer-Brightspeed API' && $transDone == 0){
				...
				}elseif($apiInfo->api_name=='AlphaTech-Brightspeed API' && $transDone == 0){
					$this->load->library('brightspeed');
					$brightSpeedResponse = $this->brightspeed->doTransaction($_POST, $apiInfo);
					//getPrint($brightSpeedResponse); exit;
					$nameOfApiForTrans	= $apiInfo->api_name;
				...
				}elseif($apiInfo->api_name=='Fringe Financial-BrightSpeed API' && $transDone == 0){
					$this->load->library('brightspeed');
					$brightSpeedResponse = $this->brightspeed->doTransaction($_POST, $apiInfo);
					//getPrint($brightSpeedResponse); exit;
					$nameOfApiForTrans	= $apiInfo->api_name;
				...
				}elseif($apiInfo->api_name=='Fringe Renewals-BrightSpeed API' && $transDone == 0){
					$this->load->library('brightspeed');
					$brightSpeedResponse = $this->brightspeed->doTransaction($_POST, $apiInfo);
					//getPrint($brightSpeedResponse); exit;
					$nameOfApiForTrans	= $apiInfo->api_name;
			...
			if($nameOfApiForTrans=='Khitomer'){
				$transData['transaction_status']	= 'ACCEPTED';
				$transData['transaction_msg']		= 'Accepted, Verifying Funds';
				$this->session->set_flashdata('message', $transData['transaction_msg']);
				$transData['system_transaction_status'] = strtolower($transData['transaction_status']);
			}elseif($nameOfApiForTrans=='Khitomer-Brightspeed API'){
				//getPrint($brightSpeedResponse); exit;
				if($brightSpeedResponse->success==TRUE || $brightSpeedResponse->success==1){
					$transData['transaction_status'] = 'ACCEPTED';
				...
				$transData['system_transaction_status'] = strtolower($transData['transaction_status']);
			}elseif($nameOfApiForTrans=='AlphaTech-Brightspeed API'){
				//getPrint($brightSpeedResponse); exit;
				if($brightSpeedResponse->success==TRUE || $brightSpeedResponse->success==1){
					$transData['transaction_status'] = 'ACCEPTED';
				...
				$transData['system_transaction_status'] = strtolower($transData['transaction_status']);
			}elseif($nameOfApiForTrans=='Fringe Financial-BrightSpeed API'){
				//getPrint($brightSpeedResponse); exit;
				if($brightSpeedResponse->success==TRUE || $brightSpeedResponse->success==1){
					$transData['transaction_status'] = 'ACCEPTED';
				...
				$transData['system_transaction_status'] = strtolower($transData['transaction_status']);
			}elseif($nameOfApiForTrans=='Fringe Renewals-BrightSpeed API'){
				//getPrint($brightSpeedResponse); exit;
				if($brightSpeedResponse->success==TRUE || $brightSpeedResponse->success==1){
					$transData['transaction_status'] = 'ACCEPTED';
			...
			if($nameOfApiForTrans == 'Khitomer-Brightspeed API' && ($brightSpeedResponse->success==TRUE || $brightSpeedResponse->success==1)){
			...
			if($nameOfApiForTrans == 'AlphaTech-Brightspeed API' && ($brightSpeedResponse->success==TRUE || $brightSpeedResponse->success==1)){
				$transData['emailLogo']		= 'alphatech-logo.png';
				//email to customer
				$subjectTitle = 'Your transaction receipt from KCL AlphaTech - 1-800-968-7240';
				$this->sendEmail('invoices/invoice_brightspeed _alphatech', $subjectTitle, $transData, $this->data['site_name'], 'support@khitomer.ca', $transData['billing_email']);
			}
			
			if($nameOfApiForTrans == 'Fringe Financial-BrightSpeed API' && ($brightSpeedResponse->success==TRUE || $brightSpeedResponse->success==1)){
				$transData['emailLogo']		= 'fringe_financial.png';
				//email to customer
				$subjectTitle = 'Your transaction receipt from KCL Fringe - 1-800-968-7240';
				$this->sendEmail('invoices/invoice_brightspeed_fringe_financial', $subjectTitle, $transData, $this->data['site_name'], 'support@khitomer.ca', $transData['billing_email']);
			}
			
			if($nameOfApiForTrans == 'Fringe Renewals-BrightSpeed API' && ($brightSpeedResponse->success==TRUE || $brightSpeedResponse->success==1)){
				$transData['emailLogo']		= 'renewals.png';
				//email to customer
				$subjectTitle = 'Your transaction receipt from KCL Renewals - 1-800-968-7240';
				$this->sendEmail('invoices/invoice_brightspeed_fringe_renewals', $subjectTitle, $transData, $this->data['site_name'], 'support@khitomer.ca', $transData['billing_email']);
			}
