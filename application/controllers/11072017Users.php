...
class Users extends Public_Controller {
	...
	public function processpayment(){
		...
		if ($this->form_validation->run() == false){
			...
			$transData['API_descriptor']	= $achApi->response_code;
	...
	public function paytoonotification(){
		$apiResponse = $_POST['MerchantApiResponse'];
		$updateTrans['transaction_status']			= $apiResponse['tr_status'];
		$updateTrans['system_transaction_status']	= strtolower($apiResponse['tr_status']);
		$updateTrans['transaction_msg']				= 'Transaction '.strtolower($apiResponse['tr_status']);
		
		$whereArr = array('trans_api_id'=>2, 'transaction_id'=>$apiResponse['tr_id']);
		//$this->common_model->updateData('transactions', $updateTrans, $whereArr);
		
		$to = "hafeez.iqbal@gmail.com";
		$subject = "status is".$apiResponse['tr_status'];
		$txt = "here is message ".$updateTrans['transaction_msg'];
	...
	public function achpayment(){
		...
		$this->data['activeClass']	= 'processpayment';
		$userID	= $this->session->userdata('userID');
		//getPrint($this->users_model->getAchAPI(5)); exit;
		...
		if ($this->form_validation->run() == false){
			...
			$this->directpayexpress->setCredentials($achApi);
			$expressData = array();
			...
			$transData['transaction_mode']		= $achApi->api_mode;
			$transData['currency']				= $this->input->post('currency');
			$transData['order_number']			= $this->input->post('order_number');
			$transData['order_description']		= $this->input->post('order_description');
			...
			$transData['trans_api_id']			= $achApi->api_id;
			
			//response transaction id
			$transData['transaction_status'] = 'processing';
			$transData['system_transaction_status'] = $transData['transaction_status'];
			...
			$transData['modified_by']			= $userID;
			$transData['payment_date']			= date('Y-m-d H:i:s');
			//getPrint($dpxResponse['data']); exit;
			
			$transData['transID'] = $this->common_model->insertData('transactions', array_filter($transData));
			
			/*$transData['affiliateName']		= $this->session->userdata('fullName');
			$transData['site_name']			= $this->data['site_name'];
			$transData['paymentMethod']		= 'processing';
			$transData['API_descriptor']	= $achApi->response_code;
			$transData['billingState']		= getColumnValue('states', 'name', $where='state_id='.$_POST['billing_state_id']);
			$transData['billingCountry']	= getColumnValue('country', 'name', $where='country_id='.$_POST['billing_country_id']);*/
			
			//email to customer
			//$subjectTitle = 'Invoice From '.$this->data['site_name'];
			//$this->sendEmail('invoice', $subjectTitle, $transData, $this->data['site_name'], $this->data['no_reply_email'], $transData['billing_email']);
			
			//$successMsg = '<a href="'.$dpxResponse['data']['paymentLink'].'" target="_blank"><strong>Click Here</strong></a> to enter Bank Details';
