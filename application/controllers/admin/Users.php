...
class Users extends Public_Controller {
	...
	public function index(){
		...
		if($this->uri->segment(4)!='')
			...
			$this->data['orderBy']	= $sorting['orderBy']	= 'companyName';
			
		if($this->uri->segment(5)!='')
			$this->data['sortOrder']= $sorting['sortOrder']	= $this->uri->segment(5);
		else
			$this->data['sortOrder']= $sorting['sortOrder']	= 'asc';
		//getPrint($_POST); exit;	
		if(!isset($this->data['page']) || $this->input->post('clear')=='clear'){
			...
			$this->session->unset_userdata('accountStatus');
		}
		...
		if($this->input->post('accountStatus')!=''){
			$this->session->set_userdata('accountStatus', $this->input->post('accountStatus'));
		}else{
			$this->session->set_userdata('accountStatus', 'ACTIVE');
		}
		
		$url = base_url("admin/users/index/".$this->data['orderBy'].'/'.$this->data['sortOrder'].'/');
        $totalrows = $this->users_model->countTotalUsers();
        parent::pagination_conf($this->data['page'], $url, $totalrows, 6);
		
		$this->data['listing']	= $this->users_model->getAllUsers($sorting, 100, $this->data['page']);
	...
	public function adduser(){
		...
		$this->form_validation->set_rules('merchant_affiliate_id', '', '');
		$this->form_validation->set_rules('user_account_status', 'Account Status', 'trim|required');
		if ($this->form_validation->run() == false){
			...
			$insertData['merchant_affiliate_id']	= $this->input->post('merchant_affiliate_id');
			$insertData['user_account_status']		= $this->input->post('user_account_status');
			$insertData['status']		= 1;
	...
	public function edituser(){
		...
		$this->form_validation->set_rules('merchant_affiliate_id', '', '');
		$this->form_validation->set_rules('user_account_status', 'Account Status', 'trim|required');
		$this->form_validation->set_rules('admin_comments', '', '');
		...
		if ($this->form_validation->run() == false){
			...
			$updateData['merchant_affiliate_id']	= $this->input->post('merchant_affiliate_id');
			$updateData['user_account_status']		= $this->input->post('user_account_status');
			$updateData['admin_comments']			= $this->input->post('admin_comments');
	...
	public function processpayment(){
		...
		if ($this->form_validation->run() == false){
			...
			//getPrint(userBillingInfo($_POST, false)); exit;
			$getApiList	=  $this->users_model->getUserAPI($userID, 'cc');
			//getPrint($getApiList);exit;
			$transDone	= 0;
			$nameOfApiForTrans	= '';
			...
			foreach($getApiList as $apiInfo){
				if($apiInfo->api_name=='NMI' && $transDone == 0){
					...
					elseif($apiResponse['response']==1){
						$transDone == 1;
						//$transData['transaction_mode'] = $apiInfo->api_mode;
						break;
					}
					//exit;
	...
	public function viewtransactions(){
		...
		$this->data['userID']	= $this->uri->segment(4);
		$this->data['contents'] = 'admin/users/viewtransactions';
		$this->load->view('template', $this->data);
