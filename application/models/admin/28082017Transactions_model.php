...
class Transactions_model extends CI_Model{
    ...
    public function getAllTransactions($sorting, $limit=0, $start=0){
		...
		if($this->session->userdata('card_number')!='')
			$this->db->like('transactions.card_number', $this->session->userdata('card_number'));
			
		if($this->session->userdata('expiry_date')!='')
			$this->db->where('transactions.expiry_date', $this->session->userdata('expiry_date'));
			
		if($this->session->userdata('amount')!='')
			$this->db->where('transactions.amount', $this->session->userdata('amount'));
    ...
    public function getAffiliateDetail($userID){
        $this->db->select('*');
		$this->db->from('users');
    ...
    public function viewTransactionResponse($transactionId){
        $this->db->select('*');
		$this->db->from('transactions');
		$this->db->where('id', $transactionId);
        return $this->db->get()->row();
