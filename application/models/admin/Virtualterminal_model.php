...
class Virtualterminal_model extends CI_Model {
    ...
    public function getAdminApiCredentials($merchantId){
        $this->db->select('*');
		$this->db->from('users');
		$this->db->where('user_id', $merchantId);
        $result = $this->db->get()->row();
		
		$apiInfo['merchantId']			= $result->user_id;
		$apiInfo['merchantKey']			= getColumnValue('api_keys', 'key', 'user_id='.$result->user_id);
		$apiInfo['merchantPassword']	= $result->password;
