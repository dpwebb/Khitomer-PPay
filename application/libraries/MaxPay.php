...
class MaxPay {
    ...
    function doTransaction($userData, $apiInfo){
		//echo $this->getTransactionUniqueId(); exit;
		...
		$transaction_unique_id = $this->getTransactionUniqueId().
		...
		$postData['transaction_unique_id']	= $transaction_unique_id;//mt_rand(1,99).mt_rand(1,999999);
	...
	function getTransactionUniqueId(){
		$CI =& get_instance();
    	$CI->load->database();
		$CI->db->select_max('id');
		$transId = $CI->db->get('transactions')->row()->id;
