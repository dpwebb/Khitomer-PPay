...
class Users extends Public_Controller {
	...
	public function processACH(){
		isUserLogin();
		$this->data['activeClass']	= 'processach';
		$userID	= $this->session->userdata('userID');
		...
		if ($this->form_validation->run() == false){
			...
			$transData['transaction_id']	 = $dpxResponse['data']['transaction_reference_id'];
			...
			$this->session->set_flashdata('message', $dpxResponse['data']['paymentLink']);
			redirect('users/transactions');
			
		}
	}
	public function directPayExpress($response){
		getPrint($response);
	}
	...
	public function getAllTransaction(){
		...
		foreach($allTransactions->result() as $row) {
		  ...
		  $records["data"][] = array(
			...
			'<a href="'.base_url('users/gettransactiondetail/'.$row->id).'" class="btn btn-sm btn-outline grey-salsa"><i class="fa fa-search"></i> View</a> <a href="'.base_url('users/refund/'.$row->id).'" class="btn btn-sm green btn-outline" style="margin-top:8px;"><i class="fa fa-exchange"></i> Refund</a>',
		 );
		}
